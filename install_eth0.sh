...
cp /etc/default/grub /etc/default/grub.jpde
PERMS=$(stat -c 'chmod %a %n; chown %U:%G %n' /etc/default/grub)
sed -i '/^GRUB_CMDLINE_LINUX=/Ic\GRUB_CMDLINE_LINUX="net.ifnames=0 biosdevname=0"' /etc/default/grub
eval ${PERMS}
